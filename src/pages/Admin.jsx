...
export const Admin = () => {
    ...
    useEffect((data) => {
        fetch('http://localhost:3000/alveer/', data)
            .then(res => res.json())
            .then(dataget => setdataget(dataget))
    }, []);

    const deletegeetchange = function (ids) {
        axios.delete('http://localhost:3000/alveer/' + ids)
    }

    const addFavorits = (id) => {
        axios.get('http://localhost:3000/alveer/' + id).then(
            res => axios.post('http://localhost:3000/favorits/', res.data)
        )
    }
